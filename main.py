...
if __name__ == '__main__':
    ...
    unet = UNet(in_chan=4, out_chan=4, embed_dim=1024, n_attn_heads=16, dim_head=64, conv_init_chan=256, chan_mults=(1,2,2,4),
                where_attn=(False, True, True, True), norm_groups=16)
    autoencoder = AutoencoderKL(32, latent_space_channel_dim=4, kl_penalty=1e-6)
    if args.from_ckpt is None: autoencoder.load_state_dict(torch.load("checkpoints/AE-epoch=13_rloss=0.008027.pth")['state_dict'])
